...
complex<double> pomeron_exchange::bottom_vertex(int mu, int lam_targ, int lam_rec, double s, double zs)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    complex<double> ui = kinematics->target.component(j, lam_targ, s, zs);
...
complex<double> pomeron_exchange::top_vertex(int mu, int lam_gam, int lam_vec, double s, double zs)
{
  ...
  for (int nu = 0; nu < 4; nu++)
  {
    ...
    temp1 = kinematics->initial.component(nu, "beam", s, zs);
    temp1 *= metric[nu];
    temp1 *= kinematics->eps_vec_star.component(nu, lam_vec, s, zs);
    sum1 += kinematics->eps_gamma.component(mu, lam_gam, s, zs) * temp1;

    temp2 = kinematics->eps_gamma.component(nu, lam_gam, s, zs);
    temp2 *= metric[nu];
    temp2 *= kinematics->eps_vec_star.component(nu, lam_vec, s, zs);
    sum2 += kinematics->initial.component(mu, "beam", s, zs) * temp2;
  }
...
complex<double> pomeron_exchange::eval(double s, double zs)
{
  complex<double> result = 0.;
  for (int i = 0; i < 24; i++)
  {
    result += helicity_amplitude(kinematics->helicities[i], s, zs);
  }
